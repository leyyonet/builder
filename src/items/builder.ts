import {
  ...
  BuilderLambda,
  BuilderSetItemLambda,
  BuilderSetItemMethod,
  BuilderSetterLambda,
  BuilderWithProxy,
  NewableClass,
} from "../type.js";
import { Obj, setFqn, StrKey } from "@leyyo/common";
import { PCK } from "../internal.js";

const SECURES = ["$finalize", "$callback", "$setItem"] as Array<StrKey<BuilderWithProxy<object>>>;

// noinspection JSUnusedGlobalSymbols
export class Builder {
  protected static setter<T extends Obj>(
    holder: BuilderAny<T>,
    obj: T,
    prop: keyof T,
    fn: BuilderSetItemLambda<T>,
  ): BuilderSetterLambda<T> {
  ...
  protected static _tupleProps<T extends Obj>(ins: T, tuples: Array<[keyof T, T[keyof T]]>) {
    tuples.forEach((item) => {
  ...
  protected static _objectProps<T extends Obj>(ins: T, values: Partial<T>) {
  ...
  protected static _fillWithDefaults<T extends Obj>(
    p1?: NewableClass<T> | Partial<T>,
    p2?: Partial<T> | Array<unknown>,
  ): Partial<T> {
    // class defined
  ...
  static build<T extends Obj>(data?: Partial<T>): BuilderAny<T>;
  static build<T extends Obj>(fn: NewableClass<T>): BuilderAny<T>;
  static build<T extends Obj>(fn: NewableClass<T>, args: Array<unknown>): BuilderAny<T>;
  static build<T extends Obj>(fn: NewableClass<T>, data: Partial<T>): BuilderAny<T>;

  static build<T extends Obj>(
    p1?: NewableClass<T> | Partial<T>,
    p2?: Partial<T> | Array<unknown>,
  ): BuilderAny<T> {
  ...
  static retrieve<T extends Obj>(lambda: BuilderLambda<Partial<T>>, data?: Partial<T>): Partial<T>;
  static retrieve<T extends Obj>(
    lambda: BuilderLambda<Partial<T>>,
    fn: NewableClass<T>,
  ): Partial<T>;
  static retrieve<T extends Obj>(
    lambda: BuilderLambda<Partial<T>>,
  ...
  static retrieve<T extends Obj>(
    ...
    if (typeof lambda === "function") {
      return this._readProp(
        lambda(this.build<Partial<T>>(p1 as NewableClass<T>, p2 as Partial<T>)),
      );
    }
